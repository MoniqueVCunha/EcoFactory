...
export default function Producao() {
  ...
  // Função para buscar produções e máquinas vindas do backend
  async function carregarDados() {
    try {
      setCarregando(true)
      const [resProducoes, resMaquinas] = await Promise.all([
        axios.get(`${API_URL}/producoes`), // Rota corrigida para /producoes
      ...
      // Define a primeira máquina como padrão no formulário caso existam máquinas prontas
      if (resMaquinas.data.length > 0) {
        setForm(prev => ({ ...prev, maquinaId: resMaquinas.data[0].id }))
      }
    } catch (err) {
  ...
  const nomeMaquina = (id) => maquinas.find((m) => m.id === Number(id))?.nome || '—'

  const produtividade = (produzida, esperada) => {
    if (!esperada) return 0
    return ((produzida / esperada) * 100).toFixed(0)
  }
  ...
  // Salvar nova produção no banco PostgreSQL via API
  async function salvar(e) {
    e.preventDefault()
    if (!validar()) return
    
    try {
      // Mapeia as chaves para snake_case exatamente como o seu banco e servidor esperam
      ...
      // Recarrega todos os dados direto do banco para trazer o nome_maquina atualizado via JOIN
      await carregarDados()
      
      // Resetar formulário mantendo a máquina atual selecionada
      setForm({ produto: '', quantidadeProduzida: '', quantidadeEsperada: '', maquinaId: maquinas[0]?.id || '', data: '' })
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        {carregando ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {listaFiltrada.map((p) => {
                const qtdProd = p.quantidade_produzida ?? p.quantidadeProduzida
                const qtdEsp = p.quantidade_esperada ?? p.quantidadeEsperada
                ...
                return (
                  <tr key={p.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
                    ...
                    <td className="px-5 py-3 text-slate-400">{new Date(p.data).toLocaleDateString('pt-BR')}</td>
                  </tr>
                )
              })}
              {listaFiltrada.length === 0 && (
                <tr><td colSpan={6} className="text-center text-slate-400 py-10">Nenhum registro de produção encontrado.</td></tr>
              )}
            </tbody>
          </table>
        )}
