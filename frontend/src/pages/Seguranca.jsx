...
const MOCK_OCORRENCIAS = [
  { id: 1, tipo: 'Vazamento de Óleo', local: 'Setor Usinagem - Torno A1', nivel_risco: 'Médio', data: '2026-07-18', descricao: 'Pequeno vazamento identificado no reservatório inferior.', medida_preventiva: 'Substituição da vedação' },
  { id: 2, tipo: 'Falta de EPI', local: 'Montagem', nivel_risco: 'Baixo', data: '2026-07-22', descricao: 'Operador sem óculos de proteção na área delimitada.', medida_preventiva: 'Reforço no treinamento de segurança' }
]
...
export default function Seguranca() {
  ...
  async function carregarOcorrencias() {
    try {
      setLoading(true)
      const res = await axios.get(API_OCORRENCIAS)
  ...
  async function salvar(e) {
    ...
    const novaOcorrencia = {
      ...
      medida_preventiva: form.medida_preventiva
    }

    try {
      await axios.post(API_OCORRENCIAS, novaOcorrencia)
      carregarOcorrencias()
    } catch {
  ...
  const criticos = ocorrencias.filter(o => o.nivel_risco === 'Crítico').length
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {ocorrencias.map((o) => {
                const riscoAtual = o.nivel_risco || 'Baixo'
                ...
                return (
                  <tr key={o.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
                    ...
                    <td className="px-5 py-3 text-slate-600 max-w-xs truncate">{o.medida_preventiva || '-'}</td>
                  </tr>
                )
              })}
          ...
          <form onSubmit={salvar} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setForm({ ...form, medida_preventiva: e.target.value })}
                className="w-full bg-slate-100 rounded-lg px-3 py-2 text-sm"
                rows={2}
                placeholder="Ex: Troca de vedação, reforço no treinamento..."
              />
            </div>
